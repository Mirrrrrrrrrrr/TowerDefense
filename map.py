...
class Cell():
    def __init__(self, col: int, row: int, _tileType: int = TILE_EMPTY, 
                 isOccupied: bool = False, _cellSize: int = 48, 
                 _offset_x: int = 0, _offset_y: int = 0):
        self.col = col
        self.row = row
        self._tileType = _tileType
    ...
    def is_path(self) -> bool:
        return self._tileType in (TILE_PATH, TILE_SPAWN, TILE_BASE)
    ...
    def can_place_turret(self) -> bool:
        return self._tileType == TILE_EMPTY and not self.is_occupied()
    ...
    def draw(self, surface: pg.Surface,
        ...
        if self._tileType == TILE_EMPTY:
            ...
            if self.image:
                ...
                if self._tileType == TILE_SPAWN:
                    color = COLOR_SPAWN 
                elif self._tileType == TILE_BASE: 
                    color = COLOR_BASE 
...
class Grid:
    ...
    def _build_path(self):
        ...
        # Memetakan segmen pasangan 
        for i in range(len(self.waypoints) - 1):
            c1, r1 = self.waypoints[i]
            c2, r2 = self.waypoints[i + 1]
            is_last_segment = (i == len(self.waypoints) - 2)
            # Menghubunkgan waypoints
            segment = self._segment_cells(c1, r1, c2, r2, include_end = is_last_segment)
            for (c, r) in segment:
                if (c, r):
                    cell = self.cells[r][c]

                    if (c, r) == self.waypoints[0]:
                        cell._tileType = TILE_SPAWN
                    elif (c, r) == self.waypoints[-1]:
                        cell._tileType = TILE_BASE
                    else:
                        cell._tileType = TILE_PATH
                    self.path.append(cell)
                    
        self._update_path_images()

    def _update_path_images(self):
        ...
        BITMASK_MAP = {
            ...
            12: Assets.TILE_CORNER_LD,  9: Assets.TILE_CORNER_LU,
            6: Assets.TILE_CORNER_RD,   3: Assets.TILE_CORNER_RU,
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                if cell.is_path():
                    ...
                    if row > 0 and self.cells[row-1][col].is_path(): mask += 1            # Atas
                    if col < self.cols - 1 and self.cells[row][col+1].is_path(): mask += 2 # Kanan
                    if row < self.rows - 1 and self.cells[row+1][col].is_path(): mask += 4 # Bawah
                    if col > 0 and self.cells[row][col-1].is_path(): mask += 8            # Kiri
